	...
	public class MainForm : System.Windows.Forms.Form
	{
		...
		private bool LoadVLTFile(string filename, bool autoUpdateTV)
		{
			...
			if (ParseVLT(myVlt))
			{
				...
				MenuItem menuItem = new MenuItem(myName, new EventHandler(mnuFileUnloadSub_Click));
				mnuFileUnload.MenuItems.Add(menuItem);
		...
		{
			foreach(VLTFile vlt in _vltFiles)
			{
				VLTDependency dep = vlt.GetChunk(VLTChunkId.Dependency) as VLTDependency;
				string name = dep.GetName(VLTDependency.VltFile);
				if (name == vltname)
				{
					AddToConsole("Unloading: " + name);
					foreach(MenuItem item in mnuFileUnload.MenuItems)
						if (item.Text == vltname)
						{
							mnuFileUnload.MenuItems.Remove(item);
							break;
						}
					_vltFiles.Remove(vlt);
					ReparseAllVLT();
					PopulateTreeView();	
					break;
				}
		...
		private bool ParseVLT(VLTFile vlt)
		{
			...
			foreach(VLTFile file in _vltFiles)
			{
		...
		private bool ReparseVLT(VLTFile vlt) 
		{
			...
			//catch
			{
				...
				if (_vltFiles.Contains(vlt))
					_vltFiles.Remove(vlt);

				foreach(MenuItem menuItem in mnuFileUnload.MenuItems)
				{
					if (menuItem.Text == name)
					{
						mnuFileUnload.MenuItems.Remove(menuItem);
					}
				}
			...
			foreach(VLTFile vlt in _vltFiles)
			{
				...
				if (name == vltname)
				{
					...
					foreach(VLTExpressionBlock blk in exp)
					{
						...
						{
							case VLTExpressionType.DatabaseLoadData:
                                AddToConsole("- Database");
								break;
							case VLTExpressionType.ClassLoadData:
								AddToConsole("- Class: " + HashResolver.Resolve(blk.Id));
								break;
							case VLTExpressionType.CollectionLoadData:
								VLTDataCollectionLoad colLoad = blk.Data.AsCollectionLoad();
								AddToConsole("- Row: " + HashResolver.Resolve(colLoad.ClassNameHash) 
									+ "/" + TrackRow(_db[colLoad.ClassNameHash].Data[colLoad.NameHash]));
								break;
						}
					}
					break;
		...
		private void PopulateTreeView()
		{
			...
			dbNode = tv.Nodes.Add("Database");
			dbNode.Tag = _db;

			foreach(VLTClass vltClass in _db)
			{
		...
		private string TrackRow(VLTDataRow row)
		{
			...
			if (row.CollectionLoad.ParentHash == 0)
				...
				return TrackRow(cls.Data[row.CollectionLoad.ParentHash]) + "/" + text;
		}
		...
		private void HashSearch(string text, ref ArrayList found)
		{
			...
			foreach(VLTClass vltClass in _db)
			{
				...
				foreach(VLTDataRow row in vltClass.Data)
				{
					...
					{
						matchText = text + ": Found match for row: " + HashResolver.Resolve(vltClass.ClassHash) + "/" + TrackRow(row);
						if (!found.Contains(matchText))
							found.Add(matchText);
					}
				}
		...
		private void HashSearchAll(string text)
		{
			...
			HashSearch(searchText, ref found);				// OMG_search_this
			HashSearch(searchText.ToLower(), ref found);	// omg_search_this
			HashSearch(searchText.ToUpper(), ref found);	// OMG_SEARCH_THIS
			...
			HashSearch(searchText, ref found);				// OmgSearchThis
			HashSearch(searchText.ToLower(), ref found);	// omgsearchthis
			HashSearch(searchText.ToUpper(), ref found);	// OMGSEARCHTHIS
			...
			if (found.Count > 0)
			{
				...
				AddToConsole("No matches found.");

		}

		private void DumpTree(VLTClass cls, string item)
		{
			AddToConsole(string.Format("Dumping contents of class: {0}, field: {1}", 
			...
			if (obj is VLTDataItemArray)
			{
				...
				for(int i=0; i<array.ValidCount; i++)
				{
					...
					if (obj == null)
						...
						value = obj.ToString();
					AddToConsole(string.Format("{0}[{1}]: {2}", name, i, value));
				}
			} 
			else
			{
				...
				if (obj == null)
					...
					value = obj.ToString();
				AddToConsole(string.Format("{0}: {1}", name, value));	
			}
		...
		private void ClassDumpAll()
		{
			...
			foreach(VLTClass cls in _db)
			{
				string className = ClassDumpSafeName(HashResolver.Resolve(cls.ClassHash));
				if (!ClassDumpDoExport(className))
					continue;
				sw.WriteLine("\tnamespace " + className + " {");
				
				sw.WriteLine("\t\tpublic abstract class " + ClassDumpSafeName(className + "_base") + " {");
				foreach(VLTClass.ClassField field in cls)
				{
					string fieldName = ClassDumpSafeName(HashResolver.Resolve(field.NameHash));
					if (!ClassDumpDoExport(fieldName))
						continue;
				...
				{
					int index = 0;
					string rowName = ClassDumpSafeName(HashResolver.Resolve(row.CollectionLoad.NameHash));
					if (!ClassDumpDoExport(rowName))
						continue;
		...
		private void ParseConsoleInput(string text)
		{
				...
				switch(command)
				{
					...
					case "load":
						...
						else
						{
							...
							if (fi.Exists)
							{								
								if (!LoadVLTFile(fi.FullName, true))
									AddToConsole("Failed to load file: " + fi.FullName);
							}
							else
								AddToConsole("Non existant file: " + fi.FullName);
						}
						break;
					case "unload":
						if (noArgs)
							AddToConsole("Error in command.");
						else
							UnloadVLTFile(args);
						break;
					case "reparse":
						AddToConsole("Reparsing all VLTs...");
						ReparseAllVLT();
						PopulateTreeView();
						break;
					case "cls":
					...
					case "hex":
						if (noArgs)
							...
							AddToConsole(string.Format("hex({0})=0x{1:x}", ulong.Parse(args), ulong.Parse(args)));
						break;
					case "hash":
						if (noArgs)
							AddToConsole("Error in command.");
						else
							AddToConsole(string.Format("hash({0})=0x{1:x}", args, VLTHasher.Hash(args)));
						break;
					case "hash64":
						if (noArgs)
							AddToConsole("Error in command.");
						else
							AddToConsole(string.Format("hash64({0})=0x{1:x}", args, VLTHasher.Hash64(args)));
						break;
					case "hs":
						goto case "hsearch";
					case "hsearch":
					...
					case "savehash":
						...
						else 
						{
							...
							AddToConsole("Saved used hashes list to: " + fi.FullName);
						}
						break;
					case "pwd":
						if (noArgs)
					...
					case "cd":
						if (noArgs)
							AddToConsole("Current directory: " + Directory.GetCurrentDirectory());
						else 
						{
							...
							{								
								...
								AddToConsole("Current directory: " + di.FullName);
							}
							else
							{
								AddToConsole("Directory does not exist.");
							}
						}
						break;
					case "ls":
					...
					case "dir":
						{
							...
							if (noArgs)
							{
								foreach(DirectoryInfo dinf in di.GetDirectories())
									AddToConsole("[d] " + dinf.Name);
								foreach(FileInfo fi in di.GetFiles())
									AddToConsole("    " + fi.Name);								
							} 
							else
							{
					...
					case "loadhash":
						if (File.Exists(args))
                            ...
                            AddToConsole("File does not exist.");
						break;
					case "reloadhashes":
                        HashResolver.Initialize();
						AddToConsole("Hashes reloaded.");
						break;
					case "loadedfiles":
                        ListLoadedFiles();
						break;
					...
					case "dump":
						...
						else
						{
							...
							{
								AddToConsole("You may only use this command on classes or data rows.");
							} 
							else
							{
								try
			...
			{
				AddToConsole("Exception: " + ex.ToString());
				AddToConsole("Error while executing: " + text);
			}
		...
		private void mnuFieldDump_Click(object sender, System.EventArgs e)
		{
			if (tvFields.SelectedNode != null)
			{
				...
				VLTDataItem di = node.Tag as VLTDataItem;
				DumpTree(di.DataRow, di.NameHash);					
		...
		private void tv_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
		{
			...
			if (tag is VLTClass)
			{
				...
				classGrid.DataMember = "Fields";

				CurrencyManager cm = (CurrencyManager)BindingContext[ds, "Fields"];
			...
			else if (tag is VLTDataRow)
			{
				...
				if (tvFields.SelectedNode != null)
				{
					if (tvFields.SelectedNode.Parent != null && tvFields.SelectedNode.Parent.Tag == null)
					{
						selNodeText = tvFields.SelectedNode.Parent.Text;
						selNodeIndex = tvFields.SelectedNode.Text;
					} 
					else
					{
						selNodeText = tvFields.SelectedNode.Text;
					}
				}
				...
				foreach(VLTClass.ClassField field in vltClass)
				{
					...
					if (field.IsArray)
					{
						...
						TreeNode node = tvFields.Nodes.Add(name);
						node.Tag = dataItem;
						for(int i=0; i<array.ValidCount; i++)
						{
					...
					{
						TreeNode node = tvFields.Nodes.Add(HashResolver.Resolve(field.NameHash));
						node.Tag = dataItem;
		...
		private void tvFields_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
		{
			...
			if (tag is VLTDataItem && !(tag is VLTDataItemArray))
			{
				...
				dataGrid.DataMember = "Values";

				CurrencyManager cm = (CurrencyManager)BindingContext[ds, "Values"];
				((DataView)cm.List).AllowNew = false;
				((DataView)cm.List).AllowEdit = false;
				((DataView)cm.List).AllowDelete = false;

				lblFieldType.Text = "Type: " + HashResolver.Resolve(dataItem.TypeHash);
				...
				{
					lblFieldOffset.Text = string.Format("Offset: {0}:0x{1:x}  ({2})", 
						(dataItem.InlineData ? "vlt" : "bin"), dataItem.Offset, name);					
				}
				else
				{
					lblFieldOffset.Text = "Null Data";
				}
		...
		{
			AddToConsole("VLTEdit " + Application.ProductVersion);
			AddToConsole("Copyright (C) 2005-2006, Arushan");
#if CARBON
			AddToConsole("Enabled Feature: Carbon Support");
#endif
#if SHOW_INTERNAL
			AddToConsole("Enabled Feature: Show Internals");
#endif
		...
		private void txtConsoleInput_KeyPress(object sender, System.Windows.Forms.KeyPressEventArgs e)
		{
			if (e.KeyChar == (char)13)
			{
				...
				if (text != "")
				{
					...
					txtConsoleInput.Refresh();
					AddToConsole("> " + text);
					ParseConsoleInput(text);
					txtConsoleInput.Focus();
					e.Handled = true;					
				}
			}
